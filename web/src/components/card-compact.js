...
const CardCompact = ({ property, isFavorite, onFavoriteToggle, showFavoriteButton = true, propertyLink }) => {
      ...
      <div className="relative w-1/3">
        ...
        <div className="absolute bottom-2 left-2 flex flex-col gap-1">
          {property.isPetsAllowed && (
            <span className="w-fit rounded-full bg-white/80 px-2 py-1 text-xs font-semibold text-black">Pets</span>
          )}

          {property.isParkingIncluded && (
            <span className="rounded-full bg-white/80 px-2 py-1 text-xs font-semibold text-black">Parking</span>
          )}
        </div>
      </div>
